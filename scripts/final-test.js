/**
 * Final test script for the Slack emoji dashboard
 * This script tests the entire flow from curl command to API request
 */
...
function simulateFetchSlackEmojis(curlCommand) {
  ...
  // Simulate the server-side handling
  simulateServerSideHandling(request)

  return request
}

// Simulate the server-side handling
function simulateServerSideHandling(request) {
  console.log("\n=== Server-side handling ===")

  // Ensure _x_id is in the URL
  const xIdMatch = request.url.match(/[?&]_x_id=([^&\s'"]+)/)
  if (!xIdMatch) {
    ...
    const separator = request.url.includes("?") ? "&" : "?"
    request.url = `${request.url}${separator}_x_id=${xId}`
    console.log("Added _x_id to URL on server side:", request.url)
  } else {
    console.log("Found _x_id in URL on server side:", xIdMatch[1])
  }

  // Check if this is an emoji-related endpoint
  const isEmojiEndpoint =
    request.url.includes("/emoji.") ||
    request.url.includes("/api/emoji") ||
    Boolean(request.url.match(/emoji\.[a-zA-Z]+/))

  if (!isEmojiEndpoint) {
    console.log("ERROR: Only emoji-related endpoints are supported")
    return
  }

  console.log("Emoji endpoint detected, request would be forwarded to Slack API")
  console.log("Final URL with _x_id:", request.url)

  // Check if _x_id is in the final URL
  const finalXIdMatch = request.url.match(/[?&]_x_id=([^&\s'"]+)/)
  if (finalXIdMatch) {
...
// Test with a curl command that has a malformed URL
const testCurlMalformedUrl = testCurl.replace(
  /https:\/\/urbankitchens\.slack\.com\/api\/emoji\.adminList/,
  "https://urbankitchens.slack.com/api/emoji.adminList?_x_id=",
)
console.log("\n=== Testing with curl command with malformed _x_id ===")
console.log("\nCurl command with malformed _x_id:\n", testCurlMalformedUrl)
simulateSlackRequest = simulateFetchSlackEmojis(testCurlMalformedUrl)
